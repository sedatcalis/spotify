...
export const renderCards = (songs) => {
  ...
  songs.forEach((songs) => {
    ...
    // console.log(songs);
    //kart elamanına ileride jsden erişmek için bazı verileri eklicez
    div.dataset.url = songs.hub?.actions.pop().uri;
    div.dataset.title = songs.title;
    div.dataset.photo = songs.images.coverart;
    ...
    <figure>
    <img
    src="${songs.images?.coverart}"
  />
  <div class="play">
    <i id="play-btn" class="bi bi-play-fill"></i>
  </div>
   </figure>
   <h4>${songs.subtitle}</h4>
   <p>${songs.title}</p>
